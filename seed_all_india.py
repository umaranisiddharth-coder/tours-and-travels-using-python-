"""
All-India seed: 80 cities x 10 buses x 10 routes + 5 hotels each.
Run: py seed_all_india.py
"""
import sys
...
with app.app_context():
    # Find next available bus number
    from sqlalchemy import text
    result = db.session.execute(text(
        "SELECT MAX(CAST(SUBSTRING(bus_number,4) AS UNSIGNED)) FROM buses WHERE bus_number LIKE 'SR-%'"
    )).scalar()
    bus_num = (result or 400) + 1

    tb = tr = th = tt = 0

    for idx, (city, lat, lng) in enumerate(CITIES):
        sys.stdout.write(f'\r  Processing {idx+1}/{len(CITIES)}: {city:<25}')
        sys.stdout.flush()
        ...
        for i, (bname, btype, seats, amenities, rating) in enumerate(BUS_TYPES):
            bnum = f'SR-{bus_num}'
            bus_num += 1
            ex = Bus.query.filter_by(bus_number=bnum).first()
            if ex:
                city_buses.append(ex)
                continue
        ...
        # 10 routes (nearest cities)
        dests = nearest_cities(city, 10)
        for bi, dest in enumerate(dests):
    ...
    print(f'\n\n✅ All-India seeding complete!')
    print(f'   Buses added    : {tb}')
    print(f'   Routes added   : {tr}')
    print(f'   Hotels added   : {th}')
